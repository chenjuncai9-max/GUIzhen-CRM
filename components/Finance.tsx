...
const Finance: React.FC<FinanceProps> = ({ financeRecords, transactions, onUpdate }) => {
    ...
    const handleAddRecord = async (e: React.FormEvent) => {
        ...
        await CloudStorageService.addFinanceRecord(newRecord);
        onUpdate();
        setAmount('');
        setDescription('');
    ...
    const handleDelete = async (id: string) => {
        if (confirm('删除记录?')) {
            await CloudStorageService.deleteFinanceRecord(id);
            onUpdate();
        }
    };
        ...
        reader.onload = (event) => {
            try {
                ...
                if (confirm(`解析到 ${newRecords.length} 条记录，确定导入吗？`)) {
                    // Use Batch Save
                    CloudStorageService.batchAddFinanceRecords(newRecords).then(() => {
                        onUpdate();
                        alert('导入成功');
                    });
                }
