...
const Customers: React.FC<CustomersProps> = ({ customers, onUpdate }) => {
  ...
  const confirmRedeem = (e: React.FormEvent) => {
      ...
      const success = StorageService.redeemCard(selectedCustomer.id, redeemTarget.cardId, redeemCount);
      ...
      if (success) {
          ...
          onUpdate();
          const updated = StorageService.getCustomers().find(c => c.id === selectedCustomer.id);
  ...
  const renderAsset = (card: CustomerCard) => {
      ...
      return (
          <div key={card.id} className={`p-4 rounded-xl border mb-3 flex justify-between items-center transition-all ${
